...
paypalrestsdk.configure({
  ...
  "client_id": "AfH7TPs1zAOH7g2quUjKKrN-HjQZxxTH4rM115bGpaDfGM0VjiIz_BpDcsaCWB2QuOfYxQfa0ySyEMd_",
  "client_secret": "ENcxKyAoY52ZYZumjCELN7AXl310Rwf8v4xuunSqe7OKhMAIvz058qM2MnMnudChuFEEUYPtqCnsRWip" })
    ...
    if session.get('_is_user_logged_in') is True:
        ...
        if request.method=='POST':
            ...
            try:
                ...
                a['price']=totalprice
            cur = conn.cursor()
            cur.execute('INSERT INTO cart_table (id,user_id,service_id,service_param) VALUES(%s,%s,%s,%s)', (cart_id, useremail,ser_id,json.dumps(a)))
            cur.close()
...
def orderinsert(a,order_id,service_id,i):
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute('INSERT INTO order_table(order_id,user_id,progress,items,order_sub_id)VALUES(%s,%s,%s,%s,%s)',(order_id, username, 0, json.dumps(a), i))
        cur.close()
    except pymysql.Error as e:
        return {
                   "msg": str(e)
...
def checkout():
    if request.method=='POST':
        order_id = "AER_EST_ORDER-" + str(randrange(111111, 999999))
        ...
        for i in range(len(items)):
            ...
            if autodiscount_applied==True:
                discounted_amount = json.loads(items[i][3])['price']-((autodiscount_percentage/100)*json.loads(items[i][3])['price'])
                a['autodiscountedprice'] = discounted_amount
            if couponcode_isapplied==True:
                try:
                    a['couponcode_discountedprice']=a['autodiscountedprice']-((couponcode_percentage/100)*a['autodiscountedprice'])
                    orderinsert(a,order_id,items[i][2],i)
                except:
                    a['couponcode_discountedprice']=a['price']-((couponcode_percentage/100)*json.loads(items[i][3])['price'])
                    orderinsert(a,order_id,items[i][2],i)
            else:
                orderinsert(a,order_id,items[i][2],i)
        return order_id,200
...
def my_orders():
    ...
    for i in range(len(myorders)):
        ...
        print(json.loads(myorders[i][3])['service_param']['price'])
        a = {
        ...
        try:
            ...
            a['autodiscountedprice'] = round(json.loads(myorders[i][3])['service_param']['price']-autodiscountedprice,4)
            a['paidamount'] = json.loads(myorders[i][3])['service_param']['autodiscountedprice']
        except :
            print('no autodiscount')
        ...
        try:
            ...
            a['couponcode_discountedprice']=round(json.loads(myorders[i][3])['service_param']['price']-couponcode_discountedprice,4)
        except:
            a['couponcode_discountedprice']=0
            print('no coupondiscount')
...
def vieworder():
    if request.method == 'GET':
        ...
        cur = conn.cursor()
        cur.execute('SELECT * FROM order_table WHERE user_id =%s AND order_id = %s AND order_sub_id=%s ORDER BY DATE(created_at) DESC', (session.get('email'),orderid,subid))
        myorders = cur.fetchall()
        cur.close()
        servicename = []
        paramlist = []
...
def myaccount():
    ...
    if request.method=='POST':
        ...
        cur = conn.cursor()
        cur.execute('UPDATE users SET first_name = %s,last_name=%s,sec_email=%s,country=%s,state=%s,houseno=%s,street=%s,city=%s,zipcode=%s,phone=%s WHERE email=%s',(fname,lname,secemail,country,states,hno,street,city,zip,phone,email,))
        cur.close()
...
def admin_dash():
    if session.get('_is_logged_in') is True:
        ...
        cur.close()
        totalsales = revenuechart['jan']+revenuechart['feb']+revenuechart['mar']+revenuechart['apr']+revenuechart['may']+revenuechart['jun']+revenuechart['july']+revenuechart['aug']+revenuechart['sept']+revenuechart['oct']+revenuechart['nov']+revenuechart['dec']
        return render_template('Admin/index.html',users= users,employees=employees,orders=orders,porders=porders,chartcount=chartcount,totalsales=totalsales,revenuechart=revenuechart)
    else:
...
def addblog():
    ...
    if request.method == 'POST':
        ...
        if blogimage.filename:
            ...
            cur = conn.cursor()
            cur.execute('INSERT INTO blog (blog_title,blog_description,blog_image,blog_seo_tags) VALUES (%s,%s,%s,%s)',(request.form.get('title'),request.form.get('description'),dounloadurl,request.form.get('tags')))
            cur.close()
            return {
...
def admin_add_employee_controller():
    if session.get('_is_logged_in') is True:
        if request.method == 'POST':
            ...
            cur = conn.cursor()
            cur.execute('INSERT INTO admin (username,password,role,name) VALUES (%s,%s,%s,%s)',(email,password,role,name))
...
def admin_updateorderstatus():
    if session.get('_is_logged_in') is True:
        if request.method == 'POST':
            ...
            cur = conn.cursor()
            cur.execute('UPDATE order_table SET progress = %s,assigned = %s,assigned_to=%s,assigned_by=%s,message=%s WHERE order_id=%s AND order_sub_id=%s',(status,assigned,employeeid,assignee,message,id,ordersubid))
            cur.close()
            #send mail
            ...
            mail.close()
            return redirect("/admin/vieworder?id="+id+"&ordered_user="+ordereduser+"&order_sub_id="+ordersubid)
        else:
            return "METHOD NOT ALLOWED"
    else:
        return redirect(url_for('admin_login'))
...
def admin_add_service():
    if session.get('_is_logged_in') is True:
        if request.method == "POST":
            ...
            cur = conn.cursor()
            cur.execute('INSERT INTO services (service_title,service_desc,isactive,image_url,ismulti,price) VALUES (%s,%s,%s,%s,%s,%s)',(title,desc,serviceenabled,dounloadurl,multienabled,price))
            cur.close()
...
def admin_updateservice():
    if session.get('_is_logged_in') is True:
        if request.method=='GET':
            ...
            if aboutimage.filename:
                cur = conn.cursor()
                cur.execute('UPDATE services SET service_title=%s,service_desc=%s,isactive=%s,image_url=%s,ismulti=%s,price=%s WHERE id=%s',(title,desc,serviceenabled,dounloadurl,multienabled,price,id))
                cur.close()
            else:
                cur = conn.cursor()
                cur.execute(
                    'UPDATE services SET service_title=%s,service_desc=%s,isactive=%s,ismulti=%s,price=%s WHERE id=%s',(title, desc, serviceenabled, multienabled, price, id))
                cur.close()
            return redirect(url_for('Admin_editservicepage'))
    else:
        return redirect(url_for('admin_login'))
...
def admin_manageserviceparams():
    if session.get('_is_logged_in') is True:
        if request.method=='GET':
            ...
            cur = conn.cursor()
            cur.execute('UPDATE serviceparams SET param=%s,paramtype=%s,param_heading=%s,price=%s WHERE slno=%s', (parameter,type,parameterheading,Price,paramid))
            cur.close()
...
def admin_add_service_param():
    if session.get('_is_logged_in') is True:
        if request.method=='GET':
            ...
            cur = conn.cursor()
            cur.execute('INSERT INTO serviceparams (id,param,paramtype,param_heading,price) VALUES (%s,%s,%s,%s,%s)', (id,param,param_type,param_heading,price))
            cur.close()
            return redirect('/admin/manageserviceparams?id='+id)
    else:
        return redirect(url_for('admin_login'))
...
def Usergroups_admin():
    if session.get('_is_logged_in') is True:
        ...
        cur = conn.cursor()
        cur.execute('SELECT * FROM user_groups JOIN group_wallet ON group_wallet.group_id=user_groups.group_id WHERE user_groups.role=1')
        groups = cur.fetchall()
...
def vviewgrouptransactions_admin():
    if session.get('_is_logged_in') is True:
        if request.method=='GET':
            ...
            cur.execute('SELECT amount FROM group_wallet WHERE group_id = %s',grpid)
            currbal = cur.fetchone()
            return render_template('/Admin/UsergroupsTransactions.html',transactions=transactions,currbal=currbal,grpid=grpid)
        if request.method=='POST':
...
def transactions_admin():
    if session.get('_is_logged_in') is True:
        if request.method=='GET':
            ...
            currbal = cur.fetchone()
            return render_template('/Admin/transactions.html',transactions=transactions,currbal=currbal,userid=userid)
        if request.method=='POST':
...
def chat_admin():
    if session.get('_is_logged_in') is True:
        if request.method=='GET':
            ...
            orders = cur.fetchall()
            cur = conn.cursor()
            cur.execute('SELECT * FROM chats WHERE order_id = %s AND order_sub_id = %s ORDER BY created_at DESC',(id,sub_id))
            chats = cur.fetchall()
...
def chatinsert_admin():
    if session.get('_is_logged_in') is True:
        ...
        if request.method=='POST':
            ...
            cur=conn.cursor()
            cur.execute('INSERT INTO chats (order_id,sender_id,replyer_id,subject,message,document_url,from_who,to_who,order_sub_id) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)',(id,ordereduser,myusername,subject,message,dounloadurl,1,0,subid))
            cur.close()
            # send mail
            ...
            mail.sendmail("devsandy12@gmail.com", ordereduser, content)
            mail.close()
            return redirect(url_for('chat_admin')+"?id="+id+"&ordered_user="+ordereduser+"&order_sub_id="+subid)
    else:
        return redirect(url_for('admin_login'))
...
def deliverables_admin():
    if session.get('_is_logged_in') is True:
        if request.method=='GET':
            ...
            orders = cur.fetchall()
            cur = conn.cursor()
            cur.execute('SELECT * FROM deliverables WHERE order_id = %s AND order_sub_id = %s ORDER BY created_at DESC',(id,sub_id))
            deliverables = cur.fetchall()
...
def deliverableinsert_admin():
    if session.get('_is_logged_in') is True:
        ...
        if request.method=='POST':
            ...
            cur=conn.cursor()
            cur.execute('INSERT INTO deliverables (order_id,order_sub_id,doc_name,doc_url,uploaded_by) VALUES (%s,%s,%s,%s,%s)',(id,subid,title,dounloadurl,myusername))
            cur.close()
            # send mail
            ...
            if addemail!="" or addemail!=None:
                ...
                mail.sendmail("devsandy12@gmail.com", ordereduser, content)
                mail.close()
            return redirect(url_for('deliverables_admin')+"?id="+id+"&ordered_user="+ordereduser+"&order_sub_id="+subid)
    else:
        return redirect(url_for('admin_login'))
...
def coupon_admin():
    if session.get('_is_logged_in') is True:
        ...
        if request.method=='POST':
            ...
            try:
                cur = conn.cursor()
                cur.execute('INSERT INTO coupon_code (for_user,coupon_name,coupon_code,discount_rate,is_enabled,qty,is_unlimited) VALUES(%s,%s,%s,%s,%s,%s,%s)',(usernames,couponname,couponcode,discountrate,enabled,qty,unlimited))
                cur.close()
            except pymysql.Error as e:
                print(e)
...
def autodiscount_admin():
    if session.get('_is_logged_in') is True:
        ...
        if request.method=='POST':
            ...
            message = request.form.get('message')
            cur = conn.cursor()
            cur.execute('INSERT INTO discount_group (group_name,offer_percentage,isenabled,message) VALUES (%s,%s,%s,%s)',(groupname,discountrate,enabled,message))
            cur.close()
...
def autodiscountsearch_admin():
    if session.get('_is_logged_in') is True:
        if request.method=='GET':
            ...
            cur = conn.cursor()
            query = ("SELECT * FROM `discount_group_users` JOIN discount_group ON discount_group_users.discount_group_id=discount_group.id WHERE discount_group_user_id LIKE '"+search+"%';")
            cur.execute(query)
            discount_groups = cur.fetchall()
...
def manageusersdgroup_admin():
    if session.get('_is_logged_in') is True:
        if request.method=='GET':
            ...
            cur = conn.cursor()
            cur.execute('SELECT * FROM discount_group_users JOIN users ON discount_group_users.discount_group_user_id=users.email WHERE discount_group_id=%s',dgrpid)
            users = cur.fetchall()
            return render_template('/Admin/manageusersdgroup.html',users = users)
        if request.method=='POST':
            conn = pymysql.Connect(
                host="localhost",
                port=3306,
            ...
            try:
                cur= conn.cursor()
                cur.execute('INSERT INTO discount_group_users (discount_group_id,discount_group_user_id) VALUES (%s,%s)',(dgrpid,dgrpuserid))
                cur.close()
                cur = conn.cursor()
...
def deletedgroupuser_admin():
    if session.get('_is_logged_in') is True:
        if request.method=='GET':
            ...
            try:
                cur = conn.cursor()
                cur.execute('DELETE FROM discount_group_users WHERE discount_group_id = %s AND discount_group_user_id=%s',(uid,userid))
                cur.close()
                cur = conn.cursor()
    ...
    if session.get('_is_logged_in') is True:
        if request.method=='GET':
            ...
            for i in range(len(user)):
                cur = conn.cursor()
                cur.execute('SELECT amount_details FROM transaction WHERE user_id=%s AND transaction_type=%s',(user[i][2],0))
...
def viewtransactionsingle_admin():
    if session.get('_is_logged_in') is True:
        if request.method=='GET':
            ...
            userdetails = cur.fetchone()
            return render_template('/Admin/viewtransactionsingle.html',transactions=transactions,userdetails=userdetails)
    else:
        return redirect(url_for('admin_login'))
...
def updatecustomer_admin():
    if session.get('_is_logged_in') is True:
        if request.method=='POST':
            ...
            cur = conn.cursor()
            cur.execute('UPDATE users SET first_name=%s,last_name=%s,password=%s,sec_email=%s,shipping_address=%s WHERE email=%s',(firstname,lastname,password,secondarymail,billingaddress,email))
            cur.close()
...
def updateuser():
    if request.method == 'POST':
        ...
        try:
            cur = conn.cursor()
            cur.execute('UPDATE users SET first_name=%s,last_name=%s,sec_email=%s WHERE email=%s', (first_name, last_name,sec_email,email))
            cur.close()
            return {
                   "msg" : "USER UPDATED",
               },200
...
def createcart():
    if request.method == 'POST':
        ...
        try:
            cur = conn.cursor()
            cur.execute('INSERT INTO cart_table(id,user_id,service_id,service_param)VALUES(%s,%s,%s,%s)',(cart_id,user_id,service_id,service_param))
            cur.close()
            return {
                   "msg": "CART CREATED",
               },200
...
def creategroup():
    if request.method == 'POST':
        ...
        try:
            cur = conn.cursor()
            cur.execute('INSERT INTO user_groups (group_id,group_name,user_id,role) VALUES(%s,%s,%s,%s)',(group_id,group_name, user_id,1))
            cur.close()
            cur = conn.cursor()
            cur.execute('INSERT INTO group_wallet (group_id,amount) VALUES (%s,%s)',(group_id,0))
            cur.close()
...
def add_user_into_group():
    if request.method == 'POST':
        ...
        try:
            cur = conn.cursor()
            cur.execute('INSERT INTO user_groups (user_id,group_id,group_name,role) VALUES (%s,%s,%s,%s)',(user_id,group_id,group_name,"0"))
            cur.close()
            return {
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user:
                ...
                cur.close()
                cur = conn.cursor()
                cur.execute('INSERT INTO group_wallet_transacctions (group_transaction_id,group_id,amount,transaction_type,group_user_id) VALUES(%s,%s,%s,%s,%s)',(group_transaction_id, group_id, amount, 1, user_id))
                cur.close()
                return {
...
def createtransaction():
    if request.method == 'POST':
        ...
        try:
            cur = conn.cursor()
            cur.execute('INSERT INTO transaction(transaction_id,user_id,amount_details,transaction_type)VALUES (%s,%s,%s,%s)', (transaction_id,userid,amount,transaction_type))
            cur.close()
            if transaction_type == 0:
                cur = conn.cursor()
                cur.execute('SELECT wallet_amount FROM users WHERE email = %s',userid)
